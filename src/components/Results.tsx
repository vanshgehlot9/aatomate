...
const WhatsAppVisual = () => (
  <div className="w-full h-full relative flex items-center justify-center p-8 group perspective-1000">
    ...
    >
      ...
      {/* Floating Secondary Card (Behind, offset right) */}
...
const VoiceVisual = () => (
  <div className="w-full h-full relative flex items-center justify-center p-8 group perspective-1000">
    ...
    >
      ...
      {/* Floating Activity Log (Behind, offset bottom left) */}
...
const defaultUseCases = [
  {
    ...
    bgColor: "bg-[#e5fcc2]",
    visualColor: "bg-[#d5eca4]",
    visualContent: <WhatsAppVisual />
  },
  {
    slug: "voice-agents",
    ...
    bgColor: "bg-[#f0d9ff]",
    visualColor: "bg-[#e1c5f5]",
    visualContent: <VoiceVisual />
  },
  {
  ...
  },
  {
    ...
    bgColor: "bg-[#ffecd4]",
    visualColor: "bg-[#f5deb5]",
    visualContent: <SupportVisual />
  }
];
...
export default function Results({ useCases = [], caseStudies = [] }: { useCases?: any[], caseStudies?: any[] }) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [activeIndex, setActiveIndex] = useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = useState(true);

  const displayUseCases = useCases.length > 0 ? useCases.map((uc, i) => ({
    slug: uc.slug,
    title: uc.title,
    description: uc.description,
    bgColor: uc.bg_color || visualStyles[i % visualStyles.length].bgColor,
    visualColor: uc.visual_color || visualStyles[i % visualStyles.length].visualColor,
    visualContent: visualStyles[i % visualStyles.length].visualContent
  })) : defaultUseCases;
  ...
  const handleScroll = () => {
    ...
    if (newIndex !== activeIndex && newIndex >= 0 && newIndex < displayUseCases.length) {
    ...
    if (isAutoPlaying && isInView) {
      interval = setInterval(() => {
        setActiveIndex(current => {
          const nextIndex = current + 1 >= displayUseCases.length ? 0 : current + 1;
  ...
  return (
    <section id="results" className="py-24 bg-paper-white relative">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <div className="max-w-3xl">
          <h2 className="font-display text-[40px] md:text-[56px] leading-[1.1] tracking-[-0.02em] text-midnight-ink uppercase mb-6 relative inline-block">
            ...
            EXPLORE OUR USE CASE<br />LIBRARY.
          </h2>
          <p className="text-[15px] text-ash-gray font-medium">
            Deploy AI quickly with our use case library. No more worries about where or how to start.
          </p>
        </div>
      </div>
      ...
      >
        {displayUseCases.map((item, index) => (
          <div 
            key={index}
            className={`snap-center shrink-0 w-[85vw] md:w-[75vw] max-w-[1200px] h-[580px] md:h-[600px] rounded-[32px] md:rounded-[48px] flex flex-col md:flex-row overflow-hidden relative ${item.bgColor}`}
          >
            {/* Mobile Arrow Button (Top Right) */}
            <Link href={`/case-studies`} className="absolute top-6 right-6 w-12 h-12 bg-black text-white rounded-2xl flex items-center justify-center z-30 hover:scale-105 transition-transform md:hidden shadow-lg">
            ...
            {/* Content Area */}
            <div className={`w-full md:w-1/2 p-6 md:p-16 flex flex-col justify-start md:justify-center relative z-20 shrink-0`}>
              <h3 className="font-display text-[28px] sm:text-[36px] md:text-[48px] lg:text-[56px] leading-[1.0] tracking-[-0.02em] text-midnight-ink uppercase mb-4 pr-14 md:pr-0">
                {item.title}
              </h3>
              <p className="text-[14px] md:text-[16px] text-midnight-ink/80 leading-relaxed max-w-md mb-6 md:mb-8 font-medium">
                {item.description}
              </p>
            ...
            {/* Visual Area */}
            <div className={`w-full md:w-1/2 flex-1 flex items-center justify-center relative overflow-hidden rounded-t-[32px] md:rounded-none ${item.visualColor}`}>
